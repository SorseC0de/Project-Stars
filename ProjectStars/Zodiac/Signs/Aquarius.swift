...
extension ZodiacCatalog {
    ...
    static let aquarius = ZodiacDefinition(
        ...
        passives: [
            AquariusAirborne(),
            AquariusWeightless(),
            AquariusCornerFlight(),
        ],
        zodiaction: AquariusGoneWithTheWind()
    )
}

// MARK: - Passive 1: Airborne
...
struct AquariusAirborne: ZodiacPassive {

    let displayName = "Airborne"
...
// MARK: - Passive 2: Weightless

/// *Provisional name.* Every multi-tile move is a jump, never a slide.
///
...
struct AquariusWeightless: ZodiacPassive {

    let displayName = "Weightless"
...
// MARK: - Passive 3: Corner Flight

/// *Provisional name.* Once per plane visit, landing on a corner offers a flight
/// to another one — the diagonal opposite on Terra, any corner on Astra.
///
/// - TODO: **Not implemented — needs an optional prompt**, the same
///   suspend-and-ask machinery as Capricorn's Springboard. The Astra variant also
...
struct AquariusCornerFlight: ZodiacPassive {

    /// Key this sign owns in `SignState.planeFlags`.
    static let usedThisVisitKey = "aquarius.cornerFlight"

    let displayName = "Corner Flight"
    let summary = "Once per plane visit, landing on a corner can fly you to another. (Not yet implemented.)"
}

// MARK: - Zodiaction: Gone With the Wind
...
struct AquariusGoneWithTheWind: Zodiaction {

    let displayName = "Gone With the Wind"
